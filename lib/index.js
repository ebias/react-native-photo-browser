...
export default class PhotoBrowser extends React.Component {
  ...
  _onGridPhotoTap(index) {
	if(!this.props.displaySelectionButtons){
    	this.refs.fullScreenContainer.openPage(index, false);
		this._toggleFullScreen(true);
	} else {
		this._onMediaSelection(index, !this.state.mediaList[index].selected)
	}
  }
    ...
    if (mediaList.length > 0) {
      if (enableGrid) {
        gridContainer = (
          <Animated.View
            style={{
			height: stickyFooter ? (screenHeight - TOOLBAR_HEIGHT - (Platform.OS == 'ios' ? 0 : 20) ) : screenHeight,
          ...
          >
            <GridContainer
			  ...
			  renderCustomState={renderCustomState}
    ...
    return (
      <View style={[styles.container, {
        ...
        <TopBarComponent
		  ...
		  onBack={isFullScreen ? this._onGridButtonTap: onBack}
		  customTopBarStyle={customTopBarStyle}
		  customTopBarBackIcon={customTopBarBackIcon}
        />
      </View>
    );
  }
